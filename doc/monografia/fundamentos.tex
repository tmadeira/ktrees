...
\begin{definition}[ciclo]
  Dado um grafo $G = (V, E)$, um \textbf{ciclo} em $G$ é um subgrafo de $G$ cujos vértices podem ser arranjados numa sequência cíclica de forma que dois vértices são adjacentes se eles são consecutivos na sequência e não-adjacentes caso contrário.
\end{definition}
...
A teoria da probabilidade requer que o espaço dos eventos $S \subseteq \Omega$ satisfaça três propriedades \cite{koller}:
...
\begin{definition}[distribuição de probabilidade]
  \cite{koller}
  Seja $\Omega$ um espaço amostral e $S$ um espaço de eventos. Uma distribuição de probabilidade $P$ sobre $(\Omega, S)$ é um mapeamento dos eventos em $S$ para valores reais que satisfaz as seguintes condições:
...
A continuar.
% TODO: probabilidade condicional (2.1.2.1)
% TODO: regra de Bayes (2.1.2.2)
