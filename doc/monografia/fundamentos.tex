...
\begin{definition}[rede bayesiana]
  ...
  Uma \textbf{rede bayesiana} é uma tripla $(X, G, \pi)$, onde $G = (V, E)$ é um DAG cujos vértices correspondem a variáveis em $X$ e $\pi = \{\pi_i(x_i, x_{\pi_i})\}$ é um conjunto de parâmetros numéricos especificando valores de probabilidade condicional $\pi_i(x_i, x_{\pi_i}) = P(x_i | x_{\pi_i})$ para todo vértice $i \in V$, valor $x_i \in X_i$ e atribuição $x_{\pi_i}$ para os pais $\pi_i$ de $X_i$ (em $G$).
\end{definition}
