...
O algoritmo para codificar uma \emph{$k$-tree} rotulada consiste em cinco passos. Aqui apresentamos esse algoritmo indicando onde cada um dos passos pode ser encontrado na nossa implementação.
...
\begin{algorithm}[Algoritmo de decodificação]
  ...
  \begin{step}
    A escrever. % TODO Computar $\phi$, $\bar{q}$, $x$ e $l_M$ (definidos como no algoritmo de codificação);
  \end{step}

  \begin{step}
    A escrever. % TODO Inserir o par $(0, \varepsilon)$ correspondente a $l_M$ em $S$ e decodificar $S$ para obter a árvore característica $T$;
  \end{step}

  \begin{step}
    A escrever. % TODO Reconstruir a \emph{$k$-tree} de Rényi $R_k$ a partir de $T$;
  \end{step}

  \begin{step}
    A escrever. % TODO Aplicar $\phi^{-1}$ a $R_k$ para obter $T_k$.
  \end{step}
